...
const Navigation = () => {
  ...
  return (
    <>
      {/* Modal div */}
      <div
        className={`bg-white h-44 fixed flex z-10 flex-col w-2/3 sm:w-1/3 p-3 border border-black  transition-all duration-500 top-[50%] ${
          islogOutOpen ? " right-[50%] " : " right-[-100%] "
        }  transform translate-x-1/2 -translate-y-1/2`}
      >
        <div onClick={toggleLogoutDiv} className="fixed top-0 right-0 ">
          <img
        ...
        <div className="text-center font-semibold text-xl border-b border-black pb-3">
          <p>Are you sure?</p>
        </div>
        <div className="flex h-full gap-5  justify-center items-center">
          <button
            onClick={logOut}
            className="border border-black py-1 w-1/3 hover:bg-green-400"
          >
            Yes
          </button>{" "}
          <button
            onClick={toggleLogoutDiv}
            className="border border-black py-1 w-1/3 hover:bg-red-400"
          >
            No
          </button>
        </div>
